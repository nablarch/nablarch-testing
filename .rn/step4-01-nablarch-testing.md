...
本記録が basename で引く36クラスはいずれも `3c4bd2a` の `src/main/java`・`src/test/java` 配下で1件に定まり、
重複する2つの basename は引用していない。
ピン外の `FilePathSetting.java`・`SystemTimeUtil.java`・`YamlTestDataParser.java` と、
スクラッチパッドの検証コード `Fix1*.java` は、この検算から除いてある（前2者は末尾「ピン外の引用と、その版」で版を示す）。
