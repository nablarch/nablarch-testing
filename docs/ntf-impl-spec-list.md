...
| IV-12 | `BasiDataTypeMapping` デフォルトマッピング22種（`半角英字`→`X` 等）。未知の型記号は `IllegalArgumentException` | テストデータ構造 | `BasicDataTypeMapping.java` 行30-73 |
| IV-13 | `TEST_` プレフィクス型の自動優先選択: `TEST_{baseType}` 名のデータ型が存在する場合、自動的に優先使用される | テストデータ構造 | `DataFileFragment.java` 行211-245 |
