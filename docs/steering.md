...
**サブエージェントの指摘を鵜呑みにしない。必ず一次ソース（RSTファイル・Javaソース等）を自分で確認してから取り込む/棄却を判断すること。**

具体的には以下の手順を踏む:

1. エージェントが報告した各指摘について、対象ファイルの該当箇所を自分で読んで存在を確認する
2. 既存の成果物（S-1.md 等）に同一・類似内容がすでに記載されていないか確認する
3. YAML パーサ実装に本当に関係する仕様かどうかを自分の判断で評価する
4. 上記を踏えて取り込む/棄却を決定し、取り込む場合のみ成果物に追記する
