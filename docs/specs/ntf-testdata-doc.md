...
| 項目 | Excel | YAML |
|---|---|---|
| ファイルなし時の動作 | セットアップ用テーブルデータ（SETUP_TABLE）が存在しない場合は INSERT がスキップされエラーにはならない。それ以外のデータ種別（期待値・LIST_MAP・ファイルデータ等）が存在しない場合はエラーになる。この動作は Excel・YAML 共通 | ←同左。加えて、ファイルのパースに失敗した場合もエラーになる |
