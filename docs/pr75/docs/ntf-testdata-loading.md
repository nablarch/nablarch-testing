...
利用者向けの「テストデータの書き方」（`ntf-testdata-doc.md`）が表側だとすれば、本書はその裏側──本体がどう動くか──です。読み終えると、**どの段階で何が変換・整形され、どこから元の値に戻せなくなるか**を追えるようになります。変換ツール設計（[testdata-converter-design.md](testdata-converter-design.md)）が再利用の線引きを決める前提知識でもあります。
...
この境界が本書の肝で、変換ツール設計（[testdata-converter-design.md](testdata-converter-design.md)）が「どの処理を再利用し、どの処理を持ち込まないか」を決める根拠になる──③を通すと記法のまま運べなくなるため、変換ツールは③を実行しない。
