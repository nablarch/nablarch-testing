...
加えて、現行記法は「データ行を書かない場合でも、カラム名の行は省略できない」と定めている（`nablarch-document` `ntf-yaml-support` `testdata_notation.rst:802`、`b75f1d7`・2026-08-14 追記）。上記2形はいずれも、この記法に沿わない書き方である。
...
**この案は現行記法に反しない。** `testdata_notation.rst:802` は「データ行を書かない場合でも、カラム名の行は省略できない」と定めているが、マーカーカラム行はカラム名行として実在するため、この記法を破らない。記法の変更は不要である。
...
- **`testdata_notation.rst:802` は #24 の投入に合わせて追記する。** 現行の「データ行を書かない場合でも、カラム名の行は省略できない」に対し、「**データ行が0件の場合に限り**カラム名の行を省略できる。省略した場合は0件のテーブルとして扱われる」を加える形になる。カラム名行を書く現行の書き方は一切変わらないため、**差し替えではなく追記**である
...
| 「カラム名の行は省略できない」現行記法 | `nablarch-document`（`ntf-yaml-support`）`testdata_notation.rst:802`（`b75f1d7`・2026-08-14 追記） |
