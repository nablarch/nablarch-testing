...
**要旨**: 問題1・問題2を、それぞれ独立したタスク（#23・#24）として本体に修正する。
設定による現行動作の温存は行わない（6章）。

### 10.1 #23（案）— 問題1: 0件テーブルでも DB を読む

| 項目 | 内容 |
|---|---|
| 修正箇所 | `TableData#loadData()`（`TableData.java:337-346`）の early return を削除 |
| 修正方法 | `colNames.length == 0` のとき `dbInfo.getColumns(tableName)` を SELECT 対象にする。`getColumnNames()` は変更しない（差し戻し `190cc9a` を踏まないため） |
| 影響範囲 | 期待値検証のみ。`loadData()` の呼び出し元は `Assertion.java:81` の1箇所（8.2） |
| 後方互換 | 嘘の合格をしていたテストが FAIL する。本リポジトリに該当データは0件（8.1）。加えてテーブル名の誤記が例外化する（5章） |
| 設定フラグ | **設けない**（6章） |
| 再現テスト | DB に行がある状態でカラム名0件の期待値 `TableData` を `assertTableEquals` に渡し、現状 PASS してしまうことを示す |

### 10.2 #24（案）— 問題2: 識別子行をカラム名行として消費しない

| 項目 | 内容 |
|---|---|
| 修正箇所 | `TestDataParsingTemplate` にヘッダ行読み込みヘルパを追加し、`TableDataParser.java:111`・`ListMapParser.java:80` の `readLine()` を置き換える |
| 判定条件 | `TYPE=` / `TYPE[groupId]=` の書式に厳密一致させる。`getDataType()` の前方一致を流用しない（`MESSAGE_ID` の罠。4章） |
| 影響範囲 | 表形式リーダ（Excel 等）のみ |
| 後方互換 | 実質ゼロ。該当データは現状すでに後続ブロックが失われた状態で読まれている（5章）。本リポジトリに該当は0件（8.1） |
| **注意** | これは**明文化された現行仕様の変更**である（`testdata_notation.rst:802`。9章）。未文書の不具合修正ではない |

**#24 は converter 側のマーカーカラム対応では代替できない。** マーカーカラム案（8.3）が救うのは
converter が生成した Excel だけであり、手書き Excel で識別子行が連続した場合に後続ブロックが消えて
手前のテーブルに混入する事象（4章(a)）は残る。

一方、**converter 側のマーカーカラム対応は #24 の有無と独立に必要**である。YAML の `rows: []` を
Excel へ変換するときの出力形が要るためで、#24 を適用しても、旧バージョンの NTF でも読める Excel を
出すならマーカーカラム行を出力する方が安全である。

### 10.3 実施順（案）

**#21 → #22 → #23 → #24**

#21・#22（事象3・事象4）は解説書チームからの依頼分で、後方互換への影響がなく互いに独立している。
#23・#24 は後方互換と仕様変更の判断を伴うため後に置く。

### 10.4 本書のスコープ外（別途対応が必要）

| 項目 | 対応先 | 前提 |
|---|---|---|
| `@Ignore` 4件の解除と `YamlTableDataBuilder.java:110-114` の FIXME 削除（3章に一覧） | `nablarch-testing-yaml`（別リポジトリ・別セッション） | #23 が本体に入り `install` された後でないと検証できない |
| マーカーカラム行の出力対応 | `nablarch-testing-converter` | #24 の要否と独立に必要 |
| 解説書の記述更新 | `nablarch-document` | 9章 |

### 10.5 チームへの確認事項

1. **#24 を仕様変更として実施してよいか。** `testdata_notation.rst:802` の記述変更を伴う
2. **対象PJのテストデータに、問題1の修正で FAIL するものが無いか。** 本リポジトリは0件だが対象PJは未走査（11章）
3. **実施順（案）でよいか**
