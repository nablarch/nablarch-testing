...
合計 171 テスト → 144 テスト実行（converter パッケージ対象）→ 全グリーン。
...
| 到達不可な防御ガードの理由が C-1.md に記録されている | OK | 上表に 6 件の根拠を記録（`System.exit()` 削除・sealed class 化・重複除去・mockStatic→実権限エラーで 4 件解消） |
