...
`YamlTestDataParser.java` は `BasicTestDataParser` を継承し（行32）、`TestDataParser` の全メソッドをオーバーライド。**`setTestDataReader()` は `UnsupportedOperationException`**（行59）で `PoiXlsReader` を一切使わない。返却型は `BasicTestDataParser` と完全同一（`getSetupTableData()`→`List<TableData>`、`getMessage()`→`MessagePool` 等）。
