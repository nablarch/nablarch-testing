...
2. **ユーザーレビュー待ち**（V-1 + C-1 をまとめて依頼）。ユーザー OK が出たらタスクリストを完了にしてブランチをマージ。
...
- `ConverterFileFilter` のメソッド名・Javadoc が `.xls` のみを指したまま（`findXlsFiles` 等）
- 設計書 6章の「`.xlsx` 変換はスコープ外」という記述が現状と乖離
- 上記をどう扱うか確認が必要（修正してコミット / 記録のみ）
