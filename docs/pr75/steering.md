...
# Step 4 フェーズのタスク（解説書を正として不一致を全件洗い出す。実装は直さない）

**指示書が唯一の作業指示**: `nablarch-document` `ntf-yaml-support` `87a21d6`:`.rn/20260724-ntf-yaml-support/ntf-step4-01-nablarch-testing.md`
...
**このフェーズ専用ルール（指示書 §1・§3・§7 より）**

- **実装は直さない。`src/main` を変更しない**（user 判断 2026-08-26）。解説書と実装が食い違い、**解説書側が正しいと判断した場合も同じ**。実装を直しに行かず、根拠を添えて報告して止める。理由は `nablarch-testing` が既にリリース済みであり、実装を動かすと利用者の後方互換が壊れること
- **「後方互換を壊さずに直せる」と自分で判断して着手しない。その判断自体が報告の対象**
- **§4-2 では `src/test` も変更しない。** 不一致が疑われる現行挙動を特性テストで固定すると、誤っている疑いのある挙動を正解として確定させることになる。§4-2 の成果物は全件表と報告だけ。**§4-1 の特性テストと §4-3 のカバレッジのテスト追加は、この禁止の対象外**
- 解説書は直さない。解説書側が誤っていると判断した項目は、根拠を添えて報告して止める
- `main`・`develop` は対象外。PR ブランチ `convert-testdata-excel-to-text` 上で作業し push する
- 「テストが通る」を完了条件にしない。完了条件2の〈負のテスト〉を実行結果で示す
...
**Steps**（指示書のページ通し順。1ページ終えるごとに表を追記してコミットする）。判定は〈一致／**不一致（解説書が正）**／**不一致（解説書側の誤りの疑い）**〉の3値で、処置は**いずれも報告のみ**:

- [x] 母集合を固定する（38ファイル・9,822行。`git ls-tree -r 40b9c52` と各ファイルの `wc -l` で実測）
- [x] `about/index.rst`（112行）→ `d9d5cc7`（是正 `dd64b92`）。行レベル分割後は表39行・対象13件・一部対象外4件・対象外22件・全件一致
- [x] `setup/index.rst`（21行）→ `ecd7903`。対象0件（toctree のみ）
- [x] `setup/common.rst`（259行）→ `608091e`。対象21件・一部対象外8件・対象外34件。不一致2件（`:126`・`:225`）／判定保留3件（`:39`・`:89`・`:166`）
...
- [ ] `index.rst`（13行）
- [ ] 「不一致」と判定した項目は**どちらの側が誤りでも実装もテストも変更せず**、報告に必要な3点（解説書の当該記述の逐語＋`file:line`＋`40b9c52` / 実装での実測結果＋`file:line`＋`3c4bd2a`。動かした場合は手順と出力 / どちらの側が誤っていると考えるかとその理由）を揃える（完了条件4）
- [ ] §4-2 を理由とする `src/main`・`src/test` の変更が0件であることを確認する（完了条件5）
...
- [ ] §6 の4観点（**A 充足／B 整合／C 規約／D 検証の妥当性**）を、**それぞれ別のサブエージェント**で回す
- [ ] §4-1 の特性テストを **観点D** で回す
- [ ] §4-2 の全件表を **観点A・観点D** で、別担当のサブエージェントで回す
- [ ] §4-2 の不一致の報告を **観点B のみ** で回す（§4-2 は記録だけで実装もテストも変えない。押さえるのは「不一致という判定そのものが実物と矛盾していないか」）
- [ ] レビュー対象に**本指示書自体**を明示的に含める（「指示された案そのものに反例がないか」）
- [x] ラウンド1（4観点）を実施 — A: fail（must 4／should 6／could 3）、B: fail（must 2／should 7／could 3）、C: fail（must 5／should 10／could 4）、D: fail（must 5／should 7／could 1）。却下3件
- [x] 是正ラウンド1（A・B・D の指摘）→ `02f3faf`..`b48b6d0`。是正ラウンド2（C の指摘）→ `b48b6d0`..`b8c1f2d`
- [x] ラウンド2観点「是正が指示範囲に収まっているか」 — fail（must 3／should 4／could 1）。**逸脱は0件**
- [ ] ラウンド2観点「是正が新しい欠陥を生んでいないか」 — **未実施**（着手直後に停止。成果物なし）
- [ ] 各ラウンドの指摘件数と観点を記録する。ラウンド2以降は差分限定の2観点だけを回す。是正ラウンドの上限は3回（**2回使用済み。残り1回**）
...
**§6 のレビューと rn 既定レビューの関係（user 判断 2026-08-26）**

- **§6 の4観点は、rn の既定レビュー（QA / Design / Craft / Verification）とは別物である。既定レビューを実施したことをもって §6 を満たしたとしない**
- rn 既定レビューを回すかどうかは任意。回す場合も **§6 の4観点に加えて**であって、代わりではない
- 理由1: 既定の4軸に**観点B に相当するものがない**。既定の Design は「構造・方針が妥当か」を見る軸で、「解説書との一致・呼び出し元・後方互換」を見る軸がない。本作業の本丸はそこである
- 理由2: 既定の QA は**完了条件ごとの gate** であり、完了条件そのものが誤っていれば素通りする。§6 が「レビューの対象に、本指示書自体を明示的に含める」と書いているのはこのため

### #34: 是正ラウンド3 — ラウンド2観点（範囲）の must 3件

**Purpose**: ラウンド2観点「是正が指示範囲に収まっているか」が挙げた must 3件を是正する。**是正ラウンドの最後の1回**にあたる。

**Prerequisites**: #30（3ページ分まで）

**Steps**:

- [ ] 表の3行が `|` 未エスケープで壊れているのを直す（`about/index.rst` の `:32`＝`dd64b92` で新規混入、`:96`・`:100`＝`608091e` から既存）。全データ行のセル数が7であることを機械的に検算する
- [ ] 「ピンからの差分」に貼った `git diff --stat 3c4bd2a HEAD` の出力が古い（記録 `1000`行/`1360 insertions`、実測 `1011`/`1371`）。再実行して差し替える。**その記述を書いたコミット自身が値を変えるため、実測は最後に取る**
- [ ] 「判定の変遷」節が不完全。ラウンド2で判定を失った4行（`about:36`・`about:38-44`・`about:112`・`common:13`）と、`common:39`・`:166` が `一致`→`判定保留` に変わったことを全件記載する
- [ ] 判定列の値のうち、指示書の列定義（3値）から外れた `判定保留`・`—`・`集約` を「指示書への指摘」に全件挙げる（現在は `集約` のみ記載）
- [ ] 是正後に、行の網羅・セル数・集計の恒等式を再検算する
...
- [ ] `git diff 3c4bd2a..HEAD --stat` の全件を表にし、`src/main/` 配下の変更が0件であることを示す（完了条件5）
...
- [ ] §8 の報告を出す（§4-1 の判定と根拠／§4-2 の集計と**不一致の全件**。「解説書が正」「解説書側の誤りの疑い」のどちらもすべて挙げる（**user の判断が要る**）／§4-3 の内訳／§6 のラウンドごとの指摘件数・観点・却下件数／完了条件8・10 の実行結果／指示から外れて行ったことの全件と理由）
...
- **Status**: paused
- **Date**: 2026-08-26
- **Last completed**: #30 のうち3ページ（`about/index.rst`・`setup/index.rst`・`setup/common.rst`）。§6 レビューはラウンド1の4観点すべてと、ラウンド2の1観点（範囲）が完了。是正ラウンドは2回実施 → `b8c1f2d`
- **Next**: **user 判断待ちが7件あり、うち3件は残り35ページの進め方を決める。** 判断が出るまで #30 の続きに着手しない。判断が出たら #34（是正ラウンド3。残り1回）→ ラウンド2の未実施観点「是正が新しい欠陥を生んでいないか」→ #30 の `setup/class_unit_test.rst` から
- **Notes**: ブランチ `convert-testdata-excel-to-text`。ピンは解説書 `40b9c52` / 本モジュール `3c4bd2a`（`git show <SHA>:<path>` で読む）。作業記録は `.rn/step4-01-nablarch-testing.md`。**user 判断待ち7件は同ファイル末尾「指示書への指摘（user 判断待ち）」と下記「未決事項」節にある。** `src/main`・`pom.xml` は `3c4bd2a` から変更0件（`src/test` の200行は §4-1 の `2470e6e` 由来）

## 未決事項（user 判断待ち。7件）

判断が出るまで #30 の残り35ページに着手しない。1〜3 は残りページ全部に効く。

| # | 論点 | 状況 | 推奨 |
|---|---|---|---|
| 1 | §2 のピンに `nablarch-testing-default-configuration` を加えるか | 解説書は `setup/request_unit_test/rest.rst:33`（`40b9c52`）でこれを「テスティングフレームワークのデフォルト設定」と名指しし、jar は `nablarch/test/test-data.config` に `nablarch.test.resource-root=src/test/java` を、`test-data.xml` に `testDataParser`＝`BasicTestDataParser`＋`PoiXlsReader` を持つ。これを見ずに `:89` を誤判定した。実体 `nablarch-testing-default-configuration-6-NEXT-20260327.002359-3.jar`（md5 `f1432ceb68d7dd0018c922f3b6e7df82`） | **加える。**バージョンの固定方法も決める（`6-NEXT-SNAPSHOT` はタイムスタンプ違いの実体が同居） |
| 2 | §4-2 の全件表に、行の網羅の検算を求めるか | §6 は「同じ表が全件であることを、その抽出方式では証明できないこと」を本当の欠陥と定義しているが、§4-2 の手順は「全量読む」としか求めず、完了条件7-3 も行の網羅を条件にしていない | **求める。**「行」列が 1..N の分割になっていることを条件にすれば機械検算できる |
| 3 | ピン外の依存（`nablarch-core` 等の jar、兄弟モジュール `nablarch-testing-yaml`）の版をどう固定するか | ピンが2つしかないため §3「事実には `file:line` と参照コミットハッシュを添える」を構造的に満たせない | 実体名（タイムスタンプ付き）＋md5 で固定する |
| 4 | 完了条件5 の測り方 | 「§4-2 を理由とする `src/main`・`src/test` の変更が0件」を求めているのに、測り方が `git diff 3c4bd2a..HEAD --stat`。`3c4bd2a` 基点では §4-1 の特性テスト（`2470e6e`）が必ず入り、stat では区別できない | §4-2 の着手点（`608091e`）を基点にするか、`git log --name-only` でコミット単位に見る |
| 5 | §4-2 「処置」列 | §4-2 は全件が「変更しない」なので定義上つねに定数。情報量がゼロで、値が割れる余地だけを生む | 列を廃し、判定に含意させる |
| 6 | §4-2 「判定」列 | 「実装での成否」列から一意に決まる（105行中98行が1:1）。別情報は「解説書が正／解説書側の誤り」の向きだけ | 成否列に向きを持たせて1列にする |
| 7 | §4-2 「対象／対象外」列 | 定義は「対象外なら理由を1行」だが、1行1記述の表に理由文を入れる設計に無理があり、理由が表外の箇条書きと二重記載になっている | 理由は表外に置くと明記する |

## 突合の現在地（38ページ中3ページ）

| 項目 | 値 |
|---|---|
| 不一致 | 2件 — `setup/common.rst:126`（解説書側の誤りの疑い）／`:225`（解説書が正＝実装側の誤りの疑い） |
| 判定保留 | 3件 — `setup/common.rst:39`・`:89`・`:166`（未決事項1 待ち） |
| 取り下げた不一致候補 | 2件 — `:89`（デフォルト設定モジュールを見ずに誤判定）／`:176`・`:252`（`println` が `null` と `"null"` を区別しないことによる偽の根拠） |
| 記録のみの副産物 | 3件 — `FixedSystemTimeProvider.java:35-36` の Javadoc が「12桁」「15桁」で実装（14/17）と食い違う／`TestEventDispatcher.java:68-74` が例外を握りつぶし `-Dnablarch.test.resource-root` が黙って無効になる構成がある／`TestDataParsingTemplate.java:136,151-152` のキャッシュ経路は未確認 |
