...
2. **V-1** に着手する（C-1-15 ユーザーレビューは V-1 完了後にまとめて依頼）

### 前回セッションで対応済みの事項（2026-06-01）

- `QuotationTrimmer` の `str.length() >= 2` ガードと境界値テスト（`testBoundaryValues`）を削除し main と差分ゼロに戻した
  - このブランチで追加されたスコープ外変更（JE-1/SW-3）だった
  - Excel の入力ルール上、1文字のダブルクォートは入力されないため、ガード自体が不要
- C-1-15 ユーザーレビューは V-1 の前に実施しても意味がないと判断
  - V-1 で変換ツール（C-1）のバグが発覚した場合、C-1 のコードが変わりレビュー対象が変わるため
  - V-1（変換ツール × YamlTestDataParser の結合テスト）を先に完了させてからまとめてレビュー依頼する

### V-1: 全 Excel テストの YAML 版並走実行（次着手タスク）

**前提**: C-1-15 ユーザーレビューより先に実施する。V-1 で C-1 に修正が入った場合、C-1 の変更も含めてまとめてユーザーレビュー依頼する。
