...
**新事象（期待値0件テーブルの偽陰性）**：上記の判定作業の中で発見した、形式共通の2問題を本体で修正する（#23・#24）。判断根拠・後方互換影響・実測結果・起票案は `docs/pr75/docs/ntf-empty-table-assertion.md` に記載。

- 問題1（#23）: 期待値のカラム名が0件だと `TableData#loadData()` が DB を読まず、DB に行が残っていても検証が必ず PASS する（偽陰性）
- 問題2（#24）: 表形式リーダが識別子行の次の行を無条件にカラム名行として読むため、識別子行が連続すると後続ブロックが消え、そのデータ行が手前のテーブルに混入する
...
- 識別子行が連続するテストデータで、後続ブロックが失われず、手前のテーブルにデータ行が混入しない（新事象・問題2）
...
- #23・#24 はいずれも**不具合修正**として起票している（`ntf-empty-table-assertion.md` 1.1）。公開中の解説書に、カラム名0件時に検証しない旨・識別子行を食う旨の記述は無い。#24 に付随する記法の拡張（0件ブロックでカラム名行を省略できる）は、未リリースの `testdata_notation.rst:802` の書き直しで足りる
- 対象PJのテストデータは未走査。#23 の修正で FAIL するデータが対象PJに無いという確証は無い（同 3.5・9章）。ただし問題2があるため該当する書き方は事実上封じられている（同 3.1）
- `nablarch-testing-yaml` の `@Ignore` 4件の解除と FIXME 削除は別リポジトリ・別セッションの作業であり、本 steering のスコープ外。#23 が本体に入り `install` された後に実施する
- `nablarch-testing-converter` のマーカーカラム行の出力対応は、本体リリースを待たない暫定策として #24 より先に必要であり、本 steering のスコープ外（同 7.1）
...
**Purpose**: 期待値のカラム名が0件のとき `TableData#loadData()` が DB を読まずに0行を返し、DB に行が残っていても検証が必ず PASS してしまう偽陰性をなくす。カラム名の宣言は「検証対象カラムの絞り込み」であって「検証するかどうか」のスイッチではない。
...
- [ ] テストが失敗することを確認し、テストのみをコミットする（コミットメッセージに「再現テストを追加する」旨を明記）
- [ ] `TableData#loadData()`（`TableData.java:337-346`）の early return を削除し、`colNames.length == 0` のとき `dbInfo.getColumns(tableName)` を SELECT 対象カラムとする。`getColumnNames()` は変更しない
- [ ] 再現テストが通ることを確認する
...
- `nablarch-testing-yaml` の `@Ignore` 4件（`ntf-empty-table-assertion.md` 3章）が解除待ちであることが、引き継ぎとして記録されている

### #24: 識別子行をカラム名行として消費しない（新事象・問題2）

**Purpose**: 表形式リーダが識別子行の次の行を無条件にカラム名行として読むため、識別子行が連続すると後続ブロックが消え、そのデータ行が手前のテーブルに混入する。次行が識別子行の書式なら消費せず、カラム名0件ブロックとして確定させる。

**Prerequisites**: #23（**逆順は禁止**。#24 を先に入れると「識別子行だけの0件ブロック」が書けるようになり、問題1に落ちて全件 PASS するため。`ntf-empty-table-assertion.md` 5.3）

**Steps**:

- [ ] 失敗する再現テストを追加する（識別子行の直後に別の識別子行が続くテストデータを読ませ、現状では後続ブロックが結果に現れず、手前のテーブルにそのデータ行が混入することを示す）
- [ ] テストが失敗することを確認し、テストのみをコミットする（コミットメッセージに「再現テストを追加する」旨を明記）
- [ ] `TestDataParsingTemplate` にヘッダ行読み込みヘルパを追加し、次行が識別子行の書式（`TYPE=` / `TYPE[groupId]=`）なら `index` を巻き戻す。`TableDataParser.java:111`・`ListMapParser.java:80` の `readLine()` を置き換える
- [ ] 再現テストが通ることを確認する
- [ ] 先頭カラム名が `MESSAGE_ID` のようにデータタイプ名で始まるテーブルが正しく読めることを、同じコミットで確認する
- [ ] 既存テストが壊れていないことを確認する（`mvn -o test` 全件。残存4Eを除く）
- [ ] 修正をコミットする

**Completion criteria**:

- 識別子行が連続するテストデータで、両方のブロックが独立して読まれ、手前のテーブルにデータ行が混入しないことが自動テストで示されている
- 識別子行がシート末尾のケース（カラム名行なし）が従来どおり0件ブロックとして読まれ、挙動が変わっていない
- 判定が `TYPE=` / `TYPE[groupId]=` の書式に限定されており、`getDataType()` の前方一致を流用していない
- `ListMapParser` 経路も同じヘルパを通っている
...
| **期待値0件テーブル偽陰性の対応方針（#23・#24 の正）** | docs/ntf-empty-table-assertion.md |
