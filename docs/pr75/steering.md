...
- #24 は `testdata_notation.rst:802` に明文化された現行仕様の変更にあたる。仕様変更として実施してよいという前提で起票している（`ntf-empty-table-assertion.md` 10.5 の確認事項1）
- 対象PJのテストデータは未走査。#23 の修正で FAIL するデータが対象PJに無いという確証は無い（同 10.5 の確認事項2）
- `nablarch-testing-yaml` の `@Ignore` 4件の解除と FIXME 削除は別リポジトリ・別セッションの作業であり、本 steering のスコープ外。#23 が本体に入り `install` された後に実施する
- `nablarch-testing-converter` のマーカーカラム行の出力対応は #24 の要否と独立に必要であり、本 steering のスコープ外
