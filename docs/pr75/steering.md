...
2. **C-1-15** ユーザーレビュー待ち → OK 取得後 **V-1** に着手（S-6 は完了済み）

### 前回セッションで対応済みの事項（2026-05-29）

- `QuotationTrimmer` の null ガードを削除（NTF の設計方針「呼び出し側が責任を持つ」に統一。他の `TestDataInterpreter` 実装に合わせた）
- スキーマ（`ntf-testdata-yaml-schema.json`）の `description` から内部実装メモ・クラス名参照を削除し、ユーザー向けに整理
- steering.md のスキーマエントリをリンクに修正

### V-1: 全 Excel テストの YAML 版並走実行（次着手タスク）

**前提**: C-1-15 ユーザーレビュー OK 取得後に着手すること。
