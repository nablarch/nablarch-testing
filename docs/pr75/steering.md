...
**目的**: リポジトリ内の全 Excel（59 件）を対象に、Excel 経由と変換 YAML 経由の読み込み結果が等価であることを確認する等価照合テストを整備する。過去に型・fw_header・フィールド名ずれ等の問題が「実 Excel を通さないテストで緑」になり見逃されたことへの再発防止が主目的。

**前提**: T1〜T6 完了・ユーザーレビュー OK 済み（型=日本語 / fw_header=messages限定 / parseMessageBlock 修正 / DataFormatter 化 / 検証モード / Doc-4 撤回）

**やり方**:
- 個々の業務テストクラスを複製しない。`ExcelToYamlEquivalenceTest` と同じ等価性照合方式で行う。
- 同一データを Excel 経由（`BasicTestDataParser`）と変換 YAML 経由（`YamlTestDataParser`）の両方で読み、結果が等価であることを照合する。
  - テーブル/LIST_MAP: テーブル名・カラム名集合・カラム数・行数・全セル値
  - ファイルデータ（固定長/可変長）: directives・各フィールド（名前/型/長さ）・データ行
  - messaging: directives・fw_header（messages のみ）・フィールド・データ行
- 検証用に作った都合のよいデータでなく、**実 Excel を変換した結果**で照合する。

**全 Excel を 3 分類して網羅する（対象を勝手に絞らない）**:

リポジトリ内の全 `.xls`/`.xlsx`（59 件）を機械的に列挙し、各ファイルを必ずいずれかに分類して一覧化する（分類漏れゼロ）。

- **分類A（環境不要で等価照合可能）**: 追加の実行環境なしで Excel/YAML 双方を読める。全件、等価照合テストを必須とする。
- **分類B（実行環境が必要）**: DB 接続等が必要なテストクラス（例: `@RunWith(DatabaseTestRunner.class)`）。等価照合テスト自体も同じ環境で実行する。**環境が無くても対象から除外しない。** 必要な環境（DB・ミドルウェア等）を具体的にユーザーに連絡し、ユーザーが環境を整えるまで待つ。環境を整えた上で全件実行しグリーンを確認する。「環境が必要だから未実施／対象外」での完了は禁止。
- **分類C（原理的に等価比較不可）**: 乱数・タイムスタンプ等で Excel と YAML を別評価すると値が一致しないもの（例: `${半角数字,...}`、タイムスタンプ）。対象外とし、ファイル単位で理由を記録する。決定的に確認できる部分は別途確認する。
- **変換対象外**: データブロックを持たないマスタ/テンプレート Excel 等は、変換対象外として理由を記録する。

**等価照合で必ず含める検証観点**（過去に問題が出た箇所）:
- 型=日本語で正しく変換されるか
- fw_header が messages のみに限定されているか
- フィールド名/型/長さの 1 列ずれが無いか
- 数値書式セルが正しく文字列化されているか

**作業内容**:
- [x] 全59件の Excel を分類A/B/C/変換対象外に機械的に分類し一覧化（分類漏れゼロを確認）
- [x] 分類A・分類B の全ファイルについて等価照合テストを作成・実行しグリーンを確認（196件グリーン）
  - 分類B で環境未整備の場合はユーザーに必要環境を連絡し、整備後に実行
- [x] 分類C・変換対象外の各ファイルに対象外理由を記録
- [x] セルフチェック（チェック結果: `docs/pr75/checks/T7.md`）
...
**完了条件**:
- リポジトリ内の全 Excel（59 件）が分類A/B/C/変換対象外のいずれかに分類され、一覧化されている（分類漏れゼロ）
- 分類A・分類B の全ファイルについて、Excel 経由と変換 YAML 経由の読み込み結果が等価であることを確認する等価照合テストが存在し、グリーンである
  - 分類B で環境が未整備の場合は、必要な環境をユーザーに連絡し、環境整備後に全件グリーンを確認する（環境問題を理由に対象から外さない）
- 分類C・変換対象外 の各ファイルは、対象外とした理由がファイル単位で記録されている
- 上記の分類一覧・結果を `docs/pr75/checks/T7.md` に記録する

**注意（過去の差し戻しを踏まえた歯止め）**:
- 「変換した」だけで完了としない。「変換 YAML を読んだ結果が Excel と等価」まで確認して完了とする。
- 対象を「型行を持つもの」「messaging のみ」等に勝手に絞らない。全 Excel を分類の対象とする。
...
2. **T7 はユーザーレビュー待ち。ユーザーからの FB を受け取り、指摘があれば対応する。**
