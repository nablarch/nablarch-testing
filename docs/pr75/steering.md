...
| **スキーマ** | `src/main/resources/nablarch/test/ntf-testdata-yaml-schema.json` | JSON Schema 定義 |
...
- [ ] **Ph-5** Excel 並走確認
  ...
  - [x] **S-6** JSON Schema 整合性確認 — schemaFullCoverage.yaml 作成・testSchemaFullCoverage 追加・仕様リストスキーマ項目列追加・旧スキーマファイル削除。ユーザーレビュー待ち
